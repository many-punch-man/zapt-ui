  ...
  <Dialog v-model="dialogVisible" :title="dialogTitle">
    ...
    >
      ...
      <el-form-item v-if="formData.type !== 3" label="显示状态" prop="visible">
        <template #label>
          <AdminTooltip message="选择隐藏时，路由将不会出现在侧边栏，但仍然可以访问" title="显示状态" />
        </template>
...
const open = async (type: string, id?: number, parentId?: number) => {
  ...
  dialogTitle.value = 'create' === type ? 'Create menu' : 'Update menu'
  formType.value = type
  resetForm()
  if (parentId) {
  ...
  if (id) {
    ...
    try {
      formData.value = await fetchGet("/system/menu/get",{
        params: {id}
      })
    } finally {
...
const submitForm = async () => {
  ...
  try {
    ...
    if (formType.value === 'create') {
      await fetchPost("/system/menu/create",{body:{
        data
        }})
      useMessage().success('Create success!')
    } else {
      await fetchPut("/system/menu/update",{body:{
        data
        }})
    ...
    formLoading.value = false
    // todo 清除localStorage中的路由信息，实现重新加载
  }
}
...
const getTree = async () => {
  menuTree.value = []
  const res = await fetchGet<any[]>("/system/menu/simple-list")
