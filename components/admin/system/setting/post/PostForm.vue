...
const dialogTitle = ref('Create position')
...
const submitForm = async () => {
  ...
  try {
    ...
    if (formType.value === 'create') {
      await fetchPost("/system/post/create", {body: data})
      useMessage().success("Create success")
    } else {
      await fetchPut("/system/post/update",{body:data})
...
const open = async (type: string, id?: number) => {
  ...
  dialogTitle.value = type === 'create' ? 'Create position' : 'Edit position'
  formType.value = type
  resetForm()
  if (id) {
    formLoading.value = true
    try {
      formData.value = await fetchGet<any>("/system/post/get", {params: {id}})
    } finally {
      formLoading.value = false
    }
  }
