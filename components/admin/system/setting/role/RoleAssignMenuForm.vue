...
import {ref} from 'vue'
defineOptions({name: 'RoleAssignMenuForm'})
...
const open = async (row: RoleVO) => {
  ...
  menuOptions.value = handleTree(await fetchGet("/system/menu/simple-list"))
  // 设置数据
  ...
  try {
    formData.value.menuIds = await fetchGet("/system/permission/list-role-menus",{params: {roleId: row.id}})
    // 设置选中
...
const submitForm = async () => {
  ...
  try {
    ...
    await fetchPost("/system/permission/assign-role-menu",{body:{
      data
    }})
